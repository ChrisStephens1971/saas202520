      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {analytics && <ExportButton data={Object.entries(analytics.metrics).map(([key, value]) => ({ Metric: key, Value: value }))} filename={`tournament-analytics-${new Date().toISOString().split('T')[0]}`} formats={['csv', 'xlsx', 'pdf']} />}
          </div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Link href="/admin/analytics/tournaments" className="px-4 py-2 bg-purple-600 text-white rounded-lg font-medium">Tournaments</Link>
        ...
        {analytics && (
          <>
            <MetricsCardGroup columns={4}>
              <MetricsCard title="Total Tournaments" value={analytics.metrics.totalTournaments} currentValue={analytics.metrics.totalTournaments} previousValue={analytics.metrics.previousTotalTournaments} format="number" />
              <MetricsCard title="Completed" value={analytics.metrics.completedTournaments} format="number" variant="success" />
              <MetricsCard title="Active Now" value={analytics.metrics.activeTournaments} format="number" variant="warning" />
              <MetricsCard title="Completion Rate" value={analytics.metrics.completionRate} currentValue={analytics.metrics.completionRate} previousValue={analytics.metrics.previousCompletionRate} format="percentage" variant="success" />
            </MetricsCardGroup>

            <MetricsCardGroup columns={4}>
              <MetricsCard title="Avg Duration" value={analytics.metrics.avgDuration} format="duration" />
              <MetricsCard title="Avg Players" value={analytics.metrics.avgPlayers} format="number" />
              <MetricsCard title="Total Matches" value={analytics.metrics.totalMatches} format="number" />
              <MetricsCard title="Avg Matches/Tournament" value={analytics.metrics.avgMatchesPerTournament} format="number" />
            </MetricsCardGroup>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <TournamentActivityChart data={analytics.charts.activity} title="Tournament Activity" />
              <FormatDistributionChart data={analytics.charts.formatDistribution} title="Tournament Formats" />
              <MatchesPerDayChart data={analytics.charts.matchesPerDay} title="Matches Per Day" />
              <MatchCompletionChart data={analytics.charts.statusDistribution} title="Tournament Status" />
            </div>
                  ...
                  <tbody>
                    {analytics.topTournaments.map((tournament) => (
                      <tr key={tournament.id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                        ...
                        <td className="text-center p-2">{tournament.duration >= 1440 ? `${(tournament.duration / 1440).toFixed(1)}d` : tournament.duration >= 60 ? `${(tournament.duration / 60).toFixed(1)}h` : `${tournament.duration}m`}</td>
                        <td className="text-center p-2">
                          <span className={`px-2 py-1 rounded-full text-xs ${tournament.completed ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <p className="text-white text-lg font-semibold capitalize">{analytics.charts.formatDistribution.length > 0 ? analytics.charts.formatDistribution[0].format.replace(/_/g, ' ') : 'N/A'}</p>
                </div>
                <div className="bg-white/5 rounded-lg p-4">
                  <p className="text-gray-400 text-sm mb-2">Match Completion Rate</p>
                  <p className="text-white text-lg font-semibold">{analytics.metrics.totalMatches > 0 ? ((analytics.metrics.completedMatches / analytics.metrics.totalMatches) * 100).toFixed(1) : '0'}%</p>
                </div>
                <div className="bg-white/5 rounded-lg p-4">
                  <p className="text-gray-400 text-sm mb-2">Avg Tournament Size</p>
                  <p className="text-white text-lg font-semibold">{analytics.metrics.avgPlayers.toFixed(0)} players</p>
                </div>
                <div className="bg-white/5 rounded-lg p-4">
                  <p className="text-gray-400 text-sm mb-2">Tournament Growth</p>
                  <p className="text-white text-lg font-semibold">{analytics.metrics.previousTotalTournaments > 0 ? ((analytics.metrics.totalTournaments - analytics.metrics.previousTotalTournaments) / analytics.metrics.previousTotalTournaments * 100).toFixed(1) : '0'}% vs previous</p>
                </div>
              </div>
            </div>
          </>
