        ...
        {analytics && (
          <>
            <MetricsCardGroup columns={4}>
              ...
              <MetricsCard
                title="New Users"
                value={analytics.metrics.newUsers}
                currentValue={analytics.metrics.newUsers}
                previousValue={analytics.metrics.previousNewUsers}
                format="number"
                variant="success"
              />
              <MetricsCard
                title="Daily Active Users"
                value={analytics.metrics.dailyActiveUsers}
                currentValue={analytics.metrics.dailyActiveUsers}
                previousValue={analytics.metrics.previousDailyActiveUsers}
                format="number"
              />
              <MetricsCard
                title="Weekly Active Users"
                value={analytics.metrics.weeklyActiveUsers}
                format="number"
            ...
            <MetricsCardGroup columns={4}>
              <MetricsCard
                ...
                value={analytics.metrics.monthlyActiveUsers}
                format="number"
                description="Last 30 days"
              />
              <MetricsCard
                title="Retention Rate"
                value={analytics.metrics.retentionRate}
                currentValue={analytics.metrics.retentionRate}
                previousValue={analytics.metrics.previousRetentionRate}
                format="percentage"
                variant="success"
              />
              <MetricsCard
                title="Avg Session Duration"
                value={analytics.metrics.avgSessionDuration}
                format="duration"
              />
              <MetricsCard
                title="Churn Rate"
                ...
                format="percentage"
                variant={analytics.metrics.churnRate < 5 ? 'success' : 'warning'}
              />
            </MetricsCardGroup>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <UserGrowthChart
                data={analytics.charts.registrations.map((r) => ({
                  date: r.date,
                  users: r.users,
                  activeUsers: 0,
                }))}
              ...
              <EngagementChart
                ...
                title="User Engagement (DAU/WAU/MAU)"
              />
              <RoleDistributionChart
                data={analytics.charts.roleDistribution}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <p className="text-white text-lg font-semibold">
                    {analytics.metrics.newUsers > 0 && analytics.metrics.previousNewUsers > 0
                      ? (
                          ((analytics.metrics.newUsers - analytics.metrics.previousNewUsers) /
                            analytics.metrics.previousNewUsers) *
                          100
                        ).toFixed(1)
                      : '0'}
                    % vs previous period
                ...
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <p className="text-white text-lg font-semibold">
                    {analytics.metrics.monthlyActiveUsers > 0
                      ? (
                          (analytics.metrics.dailyActiveUsers /
                            analytics.metrics.monthlyActiveUsers) *
                          100
                        ).toFixed(1)
                      : '0'}
                    % engagement
                ...
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <p className="text-white text-lg font-semibold">
                    {analytics.metrics.retentionRate.toFixed(1)}%
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <p className="text-white text-lg font-semibold">
                    {(
                      (analytics.metrics.dailyActiveUsers / analytics.metrics.totalUsers) *
                      100
                    ).toFixed(1)}
                    % active today
                  </p>
