...
export default function PerformanceAnalyticsPage() {
  ...
  const [selectedMetric, setSelectedMetric] = useState<'responseTime' | 'errorRate' | 'throughput'>(
    'responseTime'
  );
          ...
          <div className="flex items-center gap-4">
            ...
            {analytics && (
              <ExportButton
                ...
                filename={`performance-analytics-${new Date().toISOString().split('T')[0]}`}
          ...
          <>
            <MetricsCardGroup columns={4}>
              <MetricsCard
                ...
                value={analytics.metrics.avgResponseTime}
                currentValue={analytics.metrics.avgResponseTime}
                previousValue={analytics.metrics.previousAvgResponseTime}
                format="duration"
                variant={analytics.metrics.avgResponseTime < 100 ? 'success' : 'warning'}
              />
              <MetricsCard
                title="P95 Response Time"
                value={analytics.metrics.p95ResponseTime}
                format="duration"
                description="95th percentile"
                variant={analytics.metrics.p95ResponseTime < 200 ? 'success' : 'warning'}
              />
              <MetricsCard
                title="Error Rate"
                value={analytics.metrics.errorRate}
                currentValue={analytics.metrics.errorRate}
                previousValue={analytics.metrics.previousErrorRate}
                format="percentage"
              ...
              <MetricsCard
                ...
                format="percentage"
                variant={analytics.metrics.uptime >= 99.9 ? 'success' : 'warning'}
              />
            </MetricsCardGroup>
            ...
            <MetricsCardGroup columns={4}>
              <MetricsCard
                ...
                value={analytics.metrics.activeConnections}
                format="number"
              />
              <MetricsCard
                title="Avg Query Time"
                value={analytics.metrics.avgQueryTime}
                format="duration"
                variant={analytics.metrics.avgQueryTime < 50 ? 'success' : 'warning'}
              />
              <MetricsCard
                title="Cache Hit Rate"
                value={analytics.metrics.cacheHitRate}
                format="percentage"
                variant={analytics.metrics.cacheHitRate >= 80 ? 'success' : 'warning'}
              />
              <MetricsCard
                title="Requests/Min"
                value={analytics.metrics.requestsPerMinute}
                currentValue={analytics.metrics.requestsPerMinute}
                previousValue={analytics.metrics.previousRequestsPerMinute}
                format="number"
              />
            </MetricsCardGroup>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
              ...
              <div className="flex gap-2 mb-4">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedMetric === 'responseTime' ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}
                >
                  Response Time
                </button>
                <button
                  onClick={() => setSelectedMetric('errorRate')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedMetric === 'errorRate' ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}
                >
                  Error Rate
                </button>
                <button
                  onClick={() => setSelectedMetric('throughput')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedMetric === 'throughput' ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}
                >
                  Throughput
                </button>
              </div>
              <PerformanceChart
                data={analytics.charts.responseTime}
                title={
                    ...
                    analytics.recentErrors.map((error, i) => (
                      <div key={i} className="bg-white/5 rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          <span
                            className={`px-2 py-1 rounded text-xs font-semibold ${error.statusCode >= 500 ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}
                          >
                            {error.statusCode}
                          </span>
                  ...
                  {analytics.slowQueries.length === 0 ? (
                    ...
                    analytics.slowQueries.map((query, i) => (
                      <div key={i} className="bg-white/5 rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          <span
                            className={`px-2 py-1 rounded text-xs font-semibold ${query.avgDuration > 1000 ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}
                          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white/5 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div
                      className={`w-3 h-3 rounded-full ${analytics.metrics.errorRate < 1 && analytics.metrics.avgResponseTime < 200 ? 'bg-green-500' : analytics.metrics.errorRate < 5 ? 'bg-yellow-500' : 'bg-red-500'}`}
                    ></div>
                  </div>
                  <p className="text-white text-lg font-semibold">
                    {analytics.metrics.errorRate < 1 && analytics.metrics.avgResponseTime < 200
                      ? 'Healthy'
                      : analytics.metrics.errorRate < 5
                        ? 'Degraded'
                        : 'Critical'}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div
                      className={`w-3 h-3 rounded-full ${analytics.metrics.avgQueryTime < 50 ? 'bg-green-500' : analytics.metrics.avgQueryTime < 100 ? 'bg-yellow-500' : 'bg-red-500'}`}
                    ></div>
                  </div>
                  <p className="text-white text-lg font-semibold">
                    {analytics.metrics.avgQueryTime < 50
                      ? 'Optimal'
                      : analytics.metrics.avgQueryTime < 100
                        ? 'Acceptable'
                        : 'Slow'}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div
                      className={`w-3 h-3 rounded-full ${analytics.metrics.cacheHitRate >= 80 ? 'bg-green-500' : analytics.metrics.cacheHitRate >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`}
                    ></div>
                  </div>
                  <p className="text-white text-lg font-semibold">
                    {analytics.metrics.cacheHitRate >= 80
                      ? 'Excellent'
                      : analytics.metrics.cacheHitRate >= 60
                        ? 'Good'
                        : 'Poor'}
                  </p>
                </div>
