    ...
    <div className={`backdrop-blur-lg bg-white/10 rounded-lg border border-white/20 ${className}`}>
        ...
        <div className="px-4 pb-4 pt-2 border-t border-white/10 space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(['all', 'pending', 'ready', 'assigned', 'active', 'completed'] as const).map(
                (status) => (
                  <button
                    key={status}
          ...
          {/* Table Status Filter */}
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Table Status</label>
            <div className="flex flex-wrap gap-2">
      ...
      {hasActiveFilters && !isExpanded && (
        <div className="px-4 pb-3 flex items-center gap-2 text-xs text-gray-400">
          ...
          {!filters.showCompleted && (
            <span className="px-2 py-0.5 bg-blue-500/20 text-blue-300 rounded">Hide completed</span>
          )}
        </div>
      )}
    </div>
