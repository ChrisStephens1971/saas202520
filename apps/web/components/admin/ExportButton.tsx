...
export function ExportButton({
  ...
  const exportToExcel = async (data: ExportDataRow[], filename: string) => {
    try {
      ...
      toast.error('Failed to export to Excel. Make sure the xlsx library is installed.');
    }
  };
  ...
  const exportChartToPNG = async (chartId: string, filename: string) => {
    try {
      ...
      toast.error('Failed to export chart. Make sure html2canvas is installed.');
    }
  };
  ...
  const exportToPDF = async (data: ExportDataRow[], filename: string) => {
    try {
      ...
      toast.error('Failed to export to PDF. Make sure jspdf and jspdf-autotable are installed.');
    }
  };
...
export const exportHelpers = {
  ...
  exportMetricsSummary: (metrics: Record<string, unknown>, filename: string) => {
    ...
    button.onclick = () => {
      const csvContent = ['Metric,Value', ...data.map((row) => `${row.Metric},${row.Value}`)].join(
        '\n'
      );
